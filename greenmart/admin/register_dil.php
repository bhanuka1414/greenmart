...
<script>
    $(function(){
        ...
        if($("#fn").val().length<=0 || $("#ln").val().length<=0 || $("#add").val().length<=0 || $("#em").val().length<=0 || $("#pn").val().length<=0 || $("#us").val().length<=0 || $("#pw").val().length<=0)
        {
...
<?php 
        ...
        if (isset($_POST['reg'])) 
        {
            $sql="select id from delivery where username='{$_POST['us']}'";
            $res=mysqli_query($con, $sql);
            if(mysqli_num_rows($res)==0)
            {
                $sql22="SELECT id FROM delivery ORDER BY id DESC LIMIT 1";
...
fclose($fp);
                $pw_sh=sha1($_POST['pw']);
                $sql="INSERT INTO delivery (id,f_name,l_name,email,phone,address,type,username,password,gender,dfb,img) VALUES 
                ('{$iid}','{$_POST['fn']}','{$_POST['ln']}','{$_POST['em']}',{$_POST['pn']},'{$_POST['add']}',3,'{$_POST['us']}',
                '{$pw_sh}','{$_POST['gender']}','{$_POST['dob']}','{$data}')";
                ...
                else
                {
                    echo "<script>$(function(){
                        ...
                        $('#warn').show();
                        $('#warning').text('Registration Unsuccessful!');
                        $('html, body').animate({
                            'scrollTop' : $('#warning').position().top
                        });
                        });</script>";
...
Better Living!</p>
            ...
            <form action="" method="post" enctype="multipart/form-data" style="margin-top: 30px;">
                ...
                <input  data-format="yyyy-MM-dd" type="date" name="dob" id="dob" class="form-control" required>
                </div>
                <div class="form-group">
                    <label>Address</label>
                ...
                <input type="text" id="pn" name="pn" class="form-control" placeholder="phone no...." required>
                </div>
                <div class="form-group">
                    <label>Photo</label>
